  ...
  if (gameState === 'LOBBY') {
    return (
      <div className="min-h-screen bg-pattern relative overflow-hidden flex flex-col items-center justify-center p-6">
        ...
        <div className="absolute top-1/2 left-1/4 text-6xl opacity-10 floating" style={{ animationDelay: '1s' }}>🎲</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 w-full items-start">
            ...
            <div className="glass-card rounded-[3rem] p-10 space-y-8">
              <div className="flex items-center gap-4 border-b border-white/10 pb-4">
                 ...
                 <h3 className="text-white text-xl font-black uppercase tracking-widest">Select Realm</h3>
              </div>
                ...
                {Object.values(GameTheme).map((t) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent flex flex-col justify-end p-6">
                      <span className="text-white font-black text-lg uppercase leading-tight">{THEMES[t].name}</span>
            ...
            <div className="glass-card rounded-[3rem] p-10 flex flex-col items-center gap-8 shadow-2xl">
              <div className="text-center">
                <h3 className="text-amber-500 text-sm font-black uppercase tracking-[0.3em] mb-2">Ready to Play?</h3>
                <p className="text-white/60 text-xs font-bold px-4">Gather 2 to 4 adventurers for the ultimate race!</p>
              </div>
              ...
              <div className="flex flex-col gap-4 w-full">
                {[2, 3, 4].map((count) => (
                  ...
                  >
                    <span className="relative z-10 text-slate-950 font-black text-2xl uppercase tracking-widest">
                      {count} Players
                    </span>
              ...
              <div className="pt-4 border-t border-white/10 w-full text-center">
                 <p className="text-white/30 text-[10px] font-black uppercase tracking-widest italic">
                   Current Theme: <span className="text-white/60">{THEMES[theme].name}</span>
                 </p>
              </div>
            </div>
          </div>
        ...
        <footer className="absolute bottom-8 flex flex-col items-center gap-2">
           <div className="flex gap-4 opacity-20">
             <i className="fas fa-chess-board text-white text-xl"></i>
    ...
    <div className={`min-h-screen ${config.bg} flex flex-col items-center justify-start p-4 md:p-8 transition-all duration-1000 bg-pattern`}>
      <header className="w-full max-w-5xl flex justify-between items-center mb-8">
        <button 
          ...
          className="px-6 py-3 bg-white/5 backdrop-blur-xl text-white/50 rounded-2xl hover:text-white hover:bg-white/10 transition-all text-[11px] font-black uppercase tracking-widest border border-white/10 shadow-lg"
        >
          <i className="fas fa-chevron-left mr-2"></i> Quit Quest
        </button>
        <div className="text-center">
          <h1 className="game-title text-4xl md:text-5xl font-black select-none tracking-tighter">MYTHIC</h1>
          <div className="flex items-center justify-center gap-3">
             <div className="h-[2px] w-8 bg-amber-500/50"></div>
             <p className="text-[11px] font-black text-amber-500 uppercase tracking-[0.4em]">{config.name}</p>
             <div className="h-[2px] w-8 bg-amber-500/50"></div>
          </div>
        </div>
        <div className="w-28 flex justify-end">
           <div className="bg-amber-500/20 px-4 py-2 rounded-xl border border-amber-500/30">
              <span className="text-amber-500 font-black text-[10px] uppercase tracking-widest">Turn: {players[currentPlayerIndex]?.name}</span>
           </div>
        </div>
      </header>
      ...
      <main className="w-full max-w-7xl grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-12 items-start">
        ...
        <div className="flex flex-col gap-8 w-full sticky top-8">
          ...
          <div className="glass-card rounded-[3rem] p-10 relative overflow-hidden">
             ...
             />
             <h3 className="text-white/30 text-[10px] font-black uppercase mb-8 tracking-[0.3em]">Champion on Deck</h3>
             <div className="flex items-center gap-8">
                ...
                <div>
                  <h2 className="text-4xl font-black text-white italic tracking-tighter mb-2">{players[currentPlayerIndex]?.name}</h2>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-white/40 text-[10px] font-black uppercase tracking-widest">Goal: 100</span>
                  </div>
                </div>
             </div>
          </div>
          ...
          <div className="bg-black/40 backdrop-blur rounded-[3rem] p-8 border border-white/5">
            <div className="flex items-center gap-4 mb-8">
               <h3 className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] whitespace-nowrap">Global Standings</h3>
              ...
              {players.map((p, idx) => (
                <div 
                  ...
                  className={`
                    ...
                    ${idx === currentPlayerIndex ? 'bg-amber-500/10 border-2 border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.2)] scale-105' : 'bg-white/5 border border-transparent opacity-50'}
                  `}
                >
                  <div className="flex items-center gap-5">
                    <div className="w-8 h-8 rounded-xl bg-black/50 flex items-center justify-center text-xs font-black text-white/50 border border-white/5">{idx + 1}</div>
                    <div className="flex items-center gap-4">
                      <span className="text-2xl drop-shadow-lg" style={{ filter: `drop-shadow(0 0 8px ${p.color}88)` }}>{p.avatar}</span>
                      <span className="text-white font-black tracking-tight text-lg">{p.name}</span>
                    </div>
                  </div>
                  <div className="bg-slate-900 px-5 py-2 rounded-2xl text-amber-500 font-black text-sm border border-white/10 shadow-inner">
                    {p.position}
                  </div>
                </div>
              ))}
      ...
      {gameState === 'WINNER' && winner && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-3xl p-6">
          ...
          <div className="relative glass-card border-4 border-amber-500 rounded-[5rem] p-20 max-w-xl w-full text-center shadow-[0_0_150px_rgba(245,158,11,0.5)] transform animate-in zoom-in spin-in-1 duration-1000">
            <div className="text-[10rem] mb-12 drop-shadow-[0_20px_50px_rgba(245,158,11,0.8)]">🏆</div>
            <h1 className="game-title text-7xl font-black mb-6 italic leading-none">VICTORIOUS!</h1>
            <div className="space-y-4 mb-14">
               <p className="text-white text-3xl font-black tracking-widest uppercase">{winner.name}</p>
               <p className="text-amber-500 text-xl font-bold tracking-[0.2em] italic">Ascended to the Peak</p>
            </div>
            <button
              onClick={() => setGameState('LOBBY')}
              className="w-full bg-amber-500 hover:bg-amber-400 text-slate-950 font-black py-8 rounded-[2.5rem] transition-all shadow-[0_20px_40px_rgba(0,0,0,0.5)] active:scale-95 uppercase tracking-[0.4em] text-xl"
            >
              Begin New Journey
            </button>
          </div>
